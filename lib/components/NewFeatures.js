...
export default class NewFeatures extends Component {
        ...
        <div className={styles.body}>
          ...
          <ul className={styles.list}>
            {this.getNewFeatures(this.getUsersVersion()).map((feature, index) => {
             return <li key={index} className={styles.item}>{this.itemMarkdown(feature)}</li>
             // return <li key={index} className={styles.item}><Markdown source={this.itemMarkdown(feature)}/></li>
  ...
  itemMarkdown(item){
    let md = new Markdown('commonmark')
    return md.render(`**${item.title}** ${item.description}`)
  }
